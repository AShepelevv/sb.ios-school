...
class Navigator {
    static  func backToAuth() {
        UIApplication.shared.keyWindow?.rootViewController?.dismiss(
            animated: false,
            completion: {() in UIApplication.shared.keyWindow?.rootViewController?.present(TabBarViewControllerBuilder.openAuthorizationWindow(), animated: true, completion: nil)}
        )
    }
}
